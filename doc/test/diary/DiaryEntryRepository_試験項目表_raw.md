# 試験項目表: DiaryEntryRepository（Entitまた、検証）

| No | 観点 | テスト項目 | 入力値 | 期待結果 | 備考 |
|----|------|-----------|--------|---------|------|
| 1 | 正常系 | 全項目を指定して保存した場合、IDが自動採番される | title="test", workedTime=60, workedDate=today, contents="abc" | 保存後のidがnullでない | `@GeneratedValue` の検証 |
...
| 5 | 正常系 | 更新した場合、createdAtは変化しない | 保存済みエンティティのtitleを変更してsave | createdAt == 保存時のcreatedAt | `updatable = false` の検証 |
